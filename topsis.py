...
# You can adjust these weights based on real-world priorities
...
def apply_topsis(data_df):
    ...
    # The prompt asks for MinMax Scaling. We will scale all to [0, 1].
    scaler = MinMaxScaler()
    try:
        norm_matrix = scaler.fit_transform(matrix)
    except ValueError:
        norm_matrix = matrix # Handle case with identical values or 1 row poorly
    ...
    # For beneficial criteria (+1), Ideal is max, Anti-Ideal is min
    # For non-beneficial criteria (-1), Ideal is min, Anti-Ideal is max
...
def get_optimized_suggestions(project_data, current_score):
    ...
    for criterion in benefits:
        ...
        # Simple heuristic for expected improvement based on weights
        # (In a real app, we'd re-run TOPSIS with industry data)
